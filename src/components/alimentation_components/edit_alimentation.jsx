...
const EditAlimentationForm = (props) => {
    ...
    const [objectSelected, setObjectSelected] = useState({designation:"", quantite:""})
    ...
    const handleNewProductSubmit = (e) => {
        ...
        if(heure !== "" && quantite > 0 && ageMin > 0 && ageMax > 0 && selectedProduct !== null){
            if(ageMax > ageMin){
                ...
                if(window.confirm("Modifier le programme " + props.data?.id + " a : '" + newAlimentation.heure + "'")){
                    const response = AlimentationService.updateAlimentation(newAlimentation, props.data?.id)
                    response.then((res)=>{
    ...
    const threatAllEditData = () => {
        setHeure(props.data?.heure)
        setAgeMin(props.data?.ageMin)
        setAgeMax(props.data?.ageMax)
        setQuantite(props.data?.quantite)
        setObjectSelected(props.data?.product)
        setIsSelect(false)
        setSelectedProduct(props.data?.product["id"])
    }
                    ...
                    <div>
                        ...
                        {isSelect ? <input
                            ...
                            <div className={"mt-1 block w-full h-10 border-2 bg-gray-700 rounded-md text-sm text-white py-2 pl-2.5"}>
                                ...
                                {objectSelected.designation} : {objectSelected.quantite}kg en stock
                            </div>
...
export default EditAlimentationForm